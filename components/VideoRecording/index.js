...
const RecordingButton = ({ aspectR,setUrlAtHome }) => {
  ...
  const [initialX, setInitialX] = useState(-1*window.innerWidth/4)
  const [initialY, setInitialY] = useState(-1*window.innerHeight/8)
  const [currentX, setCurrentX] = useState(-1*window.innerWidth/4)
  const [currentY, setCurrentY] = useState(-1*window.innerHeight/8)
