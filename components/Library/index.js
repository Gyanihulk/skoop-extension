...
const Library = (props) => {
  ...
  const { globalRefresh, isLinkedin, setGlobalRefresh } =
  ...
  const updateFavoritesState = async () => {
    try {
      ...
      if (Array.isArray(linksData.links)) {
        ...
        console.error(
          "Links data's 'links' property is not an array:",
          linksData
        );
      }
    } catch (err) {
  ...
  const getlinks = async (dirName, favourite) => {
    ...
    try {
      var response = await fetch(
        API_ENDPOINTS.linkData + urlParams.toString(),
        {
          method: "GET",
          headers: {
            authorization: `Bearer ${JSON.parse(
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ display: openNewFolder ? "block" : "none" }}
      >
