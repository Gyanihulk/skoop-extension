...
const ChatComponent = (props) => {
  ...
  const [messages, setMessages] = useState([
    {
      text: 'Hi there! Click on the message box to detect receiver',
  ...
  const handleSend = (data) => {
    ...
    const newMessage = {
      text: temp.replace(/\n/g, '\n'),
      sender: 'me',
      profilePicture: 'https://w7.pngwing.com/pngs/782/115/png-transparent-avatar-boy-man-avatar-vol-1-icon-thumbnail.png',
    }
    ...
    <div className="centered-container">
        ...
        <div className="card-footer bg-white position-relative bottom-0 m-0 p-1">
          ...
          <div className="chat-window-tab">
            <ul class="nav nav-pills mb-3 justify-content-center">
              <li class="nav-item">
                <button class={`nav-link ${option === 'ChatGpt' ? 'active' : ''}`} onClick={() => handleIconClick('ChatGpt')}>
                  <RiRobot2Line className="iconButtonStyle" data-mdb-toggle="tooltip" data-mdb-placement="bottom" title="Open a chat with an AI-powered assistant, ChatGPT. You can ask questions or seek assistance." /> Chatgpt
                </button>
              </li>
              <li class="nav-item">
                <button class={`nav-link ${option === 'Giphy' ? 'active' : ''}`} onClick={() => handleIconClick('Giphy')}>
                  <AiOutlineFileGif className="iconButtonStyle" data-mdb-toggle="tooltip" data-mdb-placement="bottom" title="Search and insert GIFs into your messages to add a touch of fun and expressiveness to your conversations." /> Gif
                </button>
              </li>
              <li class="nav-item">
                <button class={`nav-link ${option === 'Library' ? 'active' : ''}`} onClick={() => handleIconClick('Library')}>
                  <MdOutlineVideoLibrary className="iconButtonStyle" data-mdb-toggle="tooltip" data-mdb-placement="bottom" title="Access your personal video library. You can find and manage your saved videos." /> Library
                </button>
              </li>
              <li class="nav-item">
                <button class={`nav-link ${option === 'Appointment' ? 'active' : ''}`} onClick={() => appendToBody(`${API_ENDPOINTS.skoopCalendarUrl}/?username=${JSON.parse(localStorage.getItem('skoopUsername'))}`)}>
                  <LuCalendarPlus className="iconButtonStyle" data-mdb-toggle="tooltip" data-mdb-placement="bottom" title="Add Appointment Meet link " /> Appointment
                </button>
              </li>
            </ul>
      ...
      {isProfilePage && (
        <div class="d-grid gap-2">
          <button class="btn btn-primary mt-3" type="button" onClick={handleOpenMessageWindow}>
            Message profile
          </button>
        </div>
      )}
