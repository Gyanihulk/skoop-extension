...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  try {
    ...
    if (request.action === 'showImagePreview') {
      let existingContainer = document.getElementById('skoop-helper-image-element')
      let existingOverlay = document.getElementById('skoop-helper-image-overlay')
      if (existingContainer) {
        // Toggle visibility if container exists
        existingContainer.remove()
        existingOverlay.remove()
      ...
      // create the container
      let container = document.createElement('div')
      container.id = 'skoop-helper-image-element'
      container.style.position = 'absolute'
      container.style.top = '0'
      container.style.left = '0'
      container.style.width = '100%'
      ...
      // create-overlay for container
      let overlay = document.createElement('div')
      overlay.id = 'skoop-helper-image-overlay'
      overlay.style.position = 'fixed'
      overlay.style.top = '0'
      overlay.style.left = '0'
      overlay.style.width = '100%'
      ...
      imageElement.id = 'skoop-helper-image'
      imageElement.src = chrome.runtime.getURL(request.src);
